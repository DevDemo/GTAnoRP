...
using System.Text;
using GTAnoRP.WEB.Models.GameViewModel;
    ...
    public class GameController : Controller
    {
        ...
        public async Task CreateCharacter(CharacterViewModel model, string socialClub)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                var response = await client.PostAsync("YOUR IP:3001/CreateCharacter", content);
        ...
        public async Task SendUserLogin(string socialclub, string token)
        {
            ...
            using (var client = new HttpClient())
            {
                string json = JsonConvert.SerializeObject(jsonData);
                var content = new StringContent(json.ToString(), Encoding.UTF8, "application/json");
                var response = await client.PostAsync("YOUR IP:3001/LoginAccount", content);
